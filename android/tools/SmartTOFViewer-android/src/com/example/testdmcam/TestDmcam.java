...
public class TestDmcam extends Activity implements OnClickListener {
	...
	public boolean devSetModfreq(dev_t dev, int freq) {
		if (dev == null)
			return false;
		...
		logUI("DMCAM",
				String.format("set freq = %d\n", wparams.getitem(0)
						.getParam_val().getMod_freq()));
		if (!dmcam.param_batch_set(dev, wparams.cast(), 1)) {
			logUI("DMCAM", String.format(" set freq to %d failed\n", freq));
			return false;
		}
		return true;
	}
